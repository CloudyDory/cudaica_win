...
void recalcFreeMem(size_t elemspercolumn) {
	...
	DPRINTF(1,"Max linear free mem = %llu (%llu, %lu)\n", mfree, nrows, ncols);
	fprintf(stdout,"Max linear free mem = %llu bytes (aprox %lu samples, %lu channels)\n", mfree, nrows, ncols);
	*/
...
void printCapabilities(cudaDeviceProp* properties) {
	...
	fprintf(stdout, "	Global Mem: %llu\n", properties->totalGlobalMem);  // original %lu
	fprintf(stdout, "	Mem: %llu\n", properties->totalGlobalMem);		   // original %lu
	fprintf(stdout, "	Mem per Block: %llu\n", properties->sharedMemPerBlock);
	fprintf(stdout, "	Regs per Block: %d\n", properties->regsPerBlock);
	fprintf(stdout, "	Warp size: %d\n", properties->warpSize);
	fprintf(stdout, "	Mem pitch: %llu\n", properties->memPitch);
