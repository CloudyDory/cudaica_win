...
% ======== test ========%
[rand_buffer,rand_index] = r250_init(1);
% ======== test ========%
...
if biasflag & extended
    [pdfperm, rand_buffer, rand_index] = initperm(datalength,rand_buffer,rand_index);
    pdfleft = datalength;
    piter = 0;
    ...
    while step < maxsteps  %%% ICA step = pass through all the data %%%%%%%%%
        ...
        % ======== test ========%
        % fprintf('Data use permutations.\n');
        [timeperm, rand_buffer, rand_index] = initperm(datalength,rand_buffer,rand_index);
        %tid = 1;
        %run_yu = cell(1,length(1:block:lastt));
        %run_w = cell(1,length(1:block:lastt));
        % ======== test ========%
        ...
        for t=1:block:lastt  %%%%%%%%% ICA Training Block %%%%%%%%%%%%%%%%%%%
            ...
            % ===== Debug only ========== %
            %tempyu = (BI-signs*y*u'-u*u');
            %fprintf("t = %d.\n",t);
            % fprintf("yu = %.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f.\n", tempyu(1), tempyu(2), tempyu(3), tempyu(4),tempyu(5), tempyu(6), tempyu(7));
            % fprintf("w  = %.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f.\n\n", weights(1), weights(2), weights(3), weights(4), weights(5), weights(6), weights(7));
            % fileID = fopen(['run_yu' num2str(t) '.txt'],'w');
            %run_yu{tid} = tempyu;
            %run_w{tid} = weights;
            %tid = tid + 1;
            % ===== Debug only ========== %
            ...
            if ~wts_blowup
                ...
                if extblocks > 0 & rem(blockno,extblocks) == 0,
                    ...
                    if kurtsize < frames % 12-22-99 rand() size suggestion by M. Spratling
                        ...
                        %=========== test ============%
                        if pdfleft < kurtsize
                            % fprintf('PDF use permutations.\n');
                            [pdfperm, rand_buffer, rand_index] = initperm(datalength,rand_buffer,rand_index);
                            piter = 0;
                            pdfleft = datalength;
                        end
                        partact=weights*double(data(:,pdfperm(piter*kurtsize+1 : (piter+1)*kurtsize)));
                        piter = piter + 1;
                        pdfleft = pdfleft - kurtsize;
                        % fprintf("Pdfperm is used at t = %d.\n",t);
                        %=========== test ============%
        ...
        if wts_blowup | isnan(change)|isinf(change),  % if weights blow up,
            ...
            icaprintf(verb,fid,'step %d - lrate %9f, wchange %8.9f, angledelta %4.1f deg\n', ...
...
fprintf('\nElapsed Infomax ICA time = %.2f seconds.\n\n',toc);
...
function [idperm, rand_buffer, rand_index] = initperm(nsamples,rand_buffer,rand_index)
idperm = 1:nsamples;

for j = nsamples:-1:1
    [rand_num,rand_buffer, rand_index] = r250(rand_buffer,rand_index);
    swap = mod(rand_num,j) + 1;
    
    if (j ~= swap)
        temp = idperm(swap);
        idperm(swap) = idperm(j);
        idperm(j) = temp;
    end
end
